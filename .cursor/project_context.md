...
- `POST /api/v1/files/upload|upload/batch`（落库返 `SysFileVO`，入参 biz/bizId/isPublic）/ `DELETE /{id}/delete` · `/delete/batch?ids=`（联动 MinIO，限本人/ALL）/ `GET /{id}/url`（私有桶统一预签名：公开不校验归属，私有校验归属）/ `GET /url?objectKey=`（统一预签名，展示用）/ `GET /page`（`@DataPermission`） - `isAuthenticated()`；业务表存 `object_key` 软关联
...
- **MemberAssetService#expireMemberPoints(memberId, now)** - 单会员过期积分清零（事务内条件置零批次+写 changeType=8 流水+原子扣 points，幂等）；`MemberPointService#expireDuePoints()` - 跨租户编排入口（逐租户 TenantContextRunner）；`MemberPointExpireJob` - 定时任务（@Scheduled，默认每天 02:00）
...
- `wj.salon.point-expire-enabled`（默认 true 启用积分过期清零）/ `wj.salon.point-expire-cron`（默认 `0 0 2 * * ?` 每天 02:00）；定时任务跨租户须 `TenantContextRunner` 逐租户切换
...
- 2026-08-17: 开通租户按字典类型同步。`TenantForm` 新增 `syncDictTypes`（类型编码列表，空则不同步字典）；`SysDictService.copyFromDefaultTenant(targetTenantId, typeCodes)` 从默认租户复制类型+字典项（目标已有该类型字典项则整体跳过，幂等；雪花 id；复制后 refreshDictCache），在 `bootstrapTenant`（TenantContextRunner 内）调用。新接口 `GET /api/v1/dict/types/options`（isAuthenticated，返回通用租户1启用类型 Option<code,name>，天然去重）。前端 TenantPage 开通表单：同步模块去掉无效的 dict 项（保留 memberLevel/memberTag），新增「同步字典」类型勾选区 + 全选/全不选按钮（打开弹窗拉取类型并默认全选）。
- 2026-08-17: 字典回到「通用(默认租户1)+租户覆盖」模式（对齐 wj-framework f3b3931 DictAspect：翻译合并通用键 `system:core:dict:{typeCode}` 与租户键 `system:core:dict:{typeCode}:{tenantId}`，同 value 租户覆盖通用，ROOT 只看通用）。本仓适配：`SystemCacheServiceImpl.refreshDictCache` 按 (typeCode,tenantId) 分键写入（租户1=通用无后缀）；`SysDictServiceImpl.listDictOptions` 通用+本租户合并（ROOT 只通用，缓存 miss 回退 DB 两段查询）；字典/字典类型分页与 `listByGroupCode` 非 ROOT 锁定本租户（原为清空 tenantId，防副本混排）。种子：`data.sql` 与 `sql/migrate-dict-tenant-overlay.sql` 为租户 2084880957290426370 补字典副本（INSERT..SELECT 自租户1，id+1000，NOT EXISTS 幂等）；**约定后续每次字典种子 SQL 必须同步加该租户副本**。迁移后重启应用（DevDataInitializer.refreshAll 重建缓存）。字典维护仍仅 ROOT；契约型字典 value 不得偏离通用。
...
- 2026-08-06: Controller 路径规则调整——`{id}` 居中（`/{id}/form`、`/{id}/detail`、`/{id}/update`、`/{id}/status`、`/{id}/password`、单删 `/{id}/delete`），新增统一 `POST /add`（不再裸 `POST` 根），删除批量 `/delete?ids=`；多级资源子动作同样 id 居中（`/types/{id}/form`、`/types/add`、`/types/{id}/update`）。后端 9 个 Controller + 前端 `systemApi.ts` 全量同步；`CLAUDE.md`/`AGENTS.md` 规范更新。原“动作前置 `/form/{id}`”规则废止
...
- 2026-07-30: 门店与部门解耦：`salon_store` 去掉 `dept_id`，新增 `salon_store_user` 维护门店-用户数据范围；`salon_member.dept_id` 改为 `store_id`；门店/会员列表和写操作按授权门店过滤；前端门店表单增加授权用户，会员表单改门店下拉
...
- 2026-07-30: 角色模型重构—ROOT=系统管理员（仅默认租户，跨租户：`WjTenantLineHandler.ignoreTable` 对 `SecurityUtils.isRoot()` 放行，查看所有租户数据）；新增 `TENANT_ADMIN`=租户管理员（每租户开通时创建，`data_scope=ALL`，菜单=除 `system:tenant:*` 外全部，管理员用户绑它）；`RoleCodes`/`data.sql`/`bootstrapTenant` 同步；菜单权限分配弹窗 `MenuPermissionDialog` 改可折叠树 + 名称/路径/权限三查询条件；主键确认=雪花（`BaseEntity @TableId(ASSIGN_ID)` + 全局 `id-type=assign_id`，int8 列，无需改）
...
- 2026-07-29: 文件落库 `sys_file` + 权限归属（列表 `@DataPermission` / 私有访问 `assertAccessible` / 删除 `assertManageable` 限本人+ALL）；CLAUDE.md/AGENTS.md 增"建表/字段必写 COMMENT"规范
