...
import type { GoodsCategoryFormPayload, GoodsCategoryPageVO } from '@/features/goods/model/goodsTypes';
...
function defaultForm(): GoodsCategoryFormPayload {
  return { name: '', sort: 0, status: 1 };
}

export function GoodsCategoryPage() {
  const hasPermission = useAuthStore((s) => s.hasPermission);
  const [rows, setRows] = useState<GoodsCategoryPageVO[]>([]);
  ...
  const [form, setForm] = useState<GoodsCategoryFormPayload>(() => defaultForm());
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const data = await goodsCategoryApi.list({
        pageNum,
        pageSize: 10,
  ...
  async function openEdit(id: string): Promise<void> {
    const detail = await goodsCategoryApi.form(id);
    setEditId(id);
    setForm({ ...detail });
    setOpen(true);
  }
  ...
  async function handleSave(): Promise<void> {
    ...
    try {
      if (editId) {
        await goodsCategoryApi.update(editId, form);
      } else {
  ...
  function setNum(field: keyof GoodsCategoryFormPayload, value: string): void {
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-zinc-500">维护洗护/造型/染护等商品分类，用于商品归类。</p>
        </div>
        {hasPermission('biz:goodsCategory:add') ? (
      ...
      {loading ? (
        ...
        <EmptyState title="暂无分类" description="还未配置任何商品分类。" />
      ) : (
              ...
              {rows.map((row) => (
                <tr key={row.id}>
                  ...
                  <td className="space-x-2 px-4 py-3 text-right">
                    {hasPermission('biz:goodsCategory:edit') ? (
                      <Button onClick={() => void openEdit(row.id)} size="sm" variant="secondary">
                        编辑
                      </Button>
                    ) : null}
                    {hasPermission('biz:goodsCategory:delete') ? (
                      <Button
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          ...
          void goodsCategoryApi.remove(confirmIds).then(async () => {
            setConfirmIds(null);
            await load();
          });
        }}
